---
title: "创建数字人"
description: "通过自然语言对话或编写 spec.yaml 创建你的 Halo 数字人。"
---
# 创建数字人
...
数字人如果需要访问要登录的网站（内部系统、企业 OA、SaaS 工具等），需要提前在**与数字人共享的浏览器 session** 中完成登录。

在数字人详情页的头部有一个 **地球图标按钮**，点击会弹出登录入口面板：

- **自定义网址输入框**：直接输入任意 URL，回车即可用共享 session 打开
- **预设入口列表**：数字人 spec 中通过 `browser_login` 字段声明的常用登录地址

在弹出的浏览器窗口中完成登录后，数字人运行时就能直接使用这个已登录的 session，无需重复操作（除非 session 过期）。

::: tip 什么时候需要用这个？
- 数字人要访问内网系统或需要账号登录的 SaaS 平台
- AI 浏览器自动化任务中需要已登录的 session
- 你自己创建的数字人默认没有预设入口，用自定义输入框即可
:::

### 在 spec 中声明预设入口

如果数字人固定需要登录某些网站，可以在 spec.yaml 中声明：

```yaml
browser_login:
  - label: "内部 OA 系统"
    url: "https://oa.company.com/login"
  - label: "数据平台"
    url: "https://data.company.com"
```

声明后这些入口会出现在地球按钮的弹出面板里，点击即开，方便团队成员快速登录。
